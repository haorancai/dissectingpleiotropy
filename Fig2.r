...
patch0 <- tibble(gamma = c(rep(0.5,n),rep(1,n))) %>% 
  ...
  geom_histogram(binwidth=.3, alpha=.7, position="identity")+
  theme_Publication()+
  ...
  xlab('Effect size')+
  ylab('Count')+
  scale_fill_manual(name = "Kurtosis", values = c('#638095','#f16c23'), labels = c('Low', 'High'))+
