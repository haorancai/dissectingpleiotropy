...
simulation_for_real_genotype <- function(NrSNP = ncol(genotype), rau = coef,
  ...
  effect <- dummy  %>% dplyr::select(V1,V2) %>% mutate_at(c('V1','V2'), as.numeric)

  effect_size_LD_correct <- dummy %>% dplyr::select(-group) %>% 
    unique() %>% 
    dplyr::select(-name) %>% 
...
simulation_for_a_given_population_exponential <- function(NrSNP = 300, rau = coef,
 ...
 #  
  ...
  # }
  n <- sample(0:10, 1)
  if (n == 0) {mode <- 'polygenic'} else {
    big_snp <- abs(effect) %>% as_tibble() %>%
  ...
  if (mode == 'major'){
      ...
      effect[big_snp,1] <- max
      if (rau > 0){
        effect[big_snp,2] <- max * sample(c(0,1), length(big_snp), replace = TRUE)
      } else {
        effect[big_snp,2] <- max * sample(c(0,-1), length(big_snp), replace = TRUE)
      }
