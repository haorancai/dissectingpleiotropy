...
load(here("data", "merged_correlation.Rfile"))

Merged_Correlations <- Merged_Correlations %>%
  mutate(Percent = case_when(
    Percent == "0uM_GdA" ~ "0µM_GdA",
...
#   #rename(rD = cor_a) %>%
#   filter(Percent != '0uM_GdA') %>%
#   ggplot(aes(abs(rD), change))+
#   ylab('Change in rG')+
#   geom_point()+
#   #scale_color_viridis(name = 'Genetic \n correlation',option="magma") +
...
p1 <- null %>%
  ...
  add_row(data %>%
    ...
    pivot_wider(names_from = "name", values_from = c("rGn", "rG"))) %>%
  mutate(
    xmin = rGn_estimate - 2 * rGn_std.error, xmax = rGn_estimate + 2 * rGn_std.error,
...
cowplot::plot_grid(
  p0 + theme(
    ...
    # legend.background = element_rect(fill="white", size=0.2, linetype="dotted", alpha = .4),
